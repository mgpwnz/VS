...
# Default variables
function="install"
version=`wget -qO- https://api.github.com/repos/subspace/subspace-cli/releases | jq '.[] | select(.prerelease==true) | select(.draft==false) | .html_url' | grep -Eo "v[0-9].[0-9].[0-9]-alpha" | head -n 1`
# Options
